        ...
        if form.is_valid():
            ...
            if "오류 발생" not in terms_text:
                ...
                if summary:  # 요약이 성공적으로 이루어진 경우
                    ...
                    if language_code != 'ko':
                        summary_in_display_language = translate_summary(summary, language_code)
                        sentiment_result = translate_summary(sentiment_result, language_code)
                    else:
                        summary_in_display_language = summary

                    return render(request, 'summary_detail.html', {
                        'summary': new_policy,
                        'summary_in_display_language': summary_in_display_language,
...
def summary_detail(request, pk):
    ...
    language_code = get_language_from_request(request, check_path=True)
    if language_code != 'ko':
        summary_in_display_language = translate_summary(summary.summary, language_code)
        sentiment_result = translate_summary(sentiment_result, language_code)
    else:
        summary_in_display_language = summary.summary
